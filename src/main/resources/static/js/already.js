...
function getorderList(){
    $.ajax({
        type:"post",
        url:"../order/searchOrderByUser.do",
        dataType:"JSON",
        data:{
            "pageNum":pageNum,
            "pageSize":pageSize,
        ...
        success:function(data){
            if(isEmptyObject(data.List)&&pageNum>0){
                ...
                for(i in list){
                    ...
                    if(list[i].state=="0"){
                        state="已预定（未付款）";
                        btnStr="<input type=\"button\"  class=\"btn btn-info\" data-roomid=\""+list[i].roomid+"\" id=\"showRoom\"  data-toggle=\"modal\" data-target=\"#showRoomT\" value=\"查看房间\"> <input type=\"button\"  class=\"btn btn-danger\"  data-orderid=\""+list[i].orderid+"\" id=\"deleteOrder\" value=\"取消\">";
                    }
