	...
	$.ajax({
		...
		success:function(data){
			if(isEmptyObject(data.List)&&pageNum>0){
				...
				$("#orderList").empty();
				$("#orderList").append("<tr><th>入住人</th><th>身份证号</th><th>电话号码</th></th><th>开始时间</th><th>结束时间</th><th>总金额</th><th>状态</th><th>操作</th></tr>")
...
function search() {
	var searchKey = document.getElementById("searchTxt").value;
...
function btnOn(){
    ...
    $("input").filter("#addIncome").on('click',function(){
        addIncome();
    });
}
...
function getIncome() {
	$.ajax({
		type:"POST",
		url:"../income/getIncomeById.do",
		dataType:"JSON",
		data:{},
		success:function(data){
			if(data.code=="0"){
				var income = data.income;
				premoney = income.tmoney;
				preroom = income.troom;
			}
			else{
				console.log("获取之前账单失败");
			}
		},
		error:function(){
			alert("获取账单发生错误")
...
function addIncome() {
	var money;
	var room;
	if(premoney==0&&preroom==0){
		money=totalmoney;
		room=totalroom;
	}else {
		money=totalmoney-premoney;
		room=totalroom-preroom;
	}
	var time = dateTime(myDate);
	$.ajax({
		type:"post",
		url:"../income/addIncome.do",
		dataType:"JSON",
		data:{
			"tmoney":money,
			"troom":room,
